...
const MORE_PROJECTS = [
  { name: "Evaluating the Effectiveness of Structured Physiotherapy with Omaveloxolone for Friedreich Ataxia Patients", period: "Sep 2025 – Nov 2025 · IIT Bombay", tags: ["Clinical Trials", "CDISC SDTM", "Database Design", "RCT"], description: "Designed an integrated clinical trial framework to evaluate combined therapy using Omaveloxolone in patients with Friedreich Ataxia. Developed a two-arm parallel RCT with power-based sample size estimation (α = 0.05, power = 80%) and clearly defined inclusion–exclusion criteria for 25+ clinical variables. Built a comprehensive Case Report Form (CRF) with 120+ data fields. Mapped 100% of trial variables to CDISC SDTM domains. Designed a normalized clinical database with 15+ relational tables.", link: "" },
  { name: "Cold Start Item Recommendation System", period: "Aug 2023 – Dec 2023 · Central University of Haryana", tags: ["Recommender Systems", "Python", "SVD", "DynamicBPR"], description: "Designed and implemented an end-to-end item cold-start recommendation system using MovieLens-200k. Implemented Attribute-Fused SVD, LearnMAP, LearnAROMA, and DynamicBPR from scratch. Developed an incremental update mechanism using DynamicBPR reducing exploration load (71% faster) while maintaining 95–97% of full model accuracy.", link: "" },
];
...
function useReveal() {
  useEffect(() => {
    ...
    const obs = new IntersectionObserver((entries) => { entries.forEach((e, i) => { if (e.isIntersecting) { setTimeout(() => e.target.classList.add("visible"), i * 60); obs.unobserve(e.target); } }); }, { threshold: 0.1, rootMargin: "0px 0px -40px 0px" });
  ...
  return (
    <div style={{ marginTop: 48 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: open ? 20 : 0 }}>
        ...
        <button onClick={() => setOpen(v => !v)} style={{ background: "none", border: "1px solid rgba(100,200,180,0.18)", borderRadius: 4, cursor: "pointer", padding: "7px 18px", display: "flex", alignItems: "center", gap: 8 }} onMouseEnter={e => e.currentTarget.style.borderColor = "rgba(77,232,194,0.45)"} onMouseLeave={e => e.currentTarget.style.borderColor = "rgba(100,200,180,0.18)"}>
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: "#4de8c2", letterSpacing: "0.1em" }}>{open ? "HIDE PROJECTS" : "MORE PROJECTS"}</span>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#4de8c2" strokeWidth="2" style={{ transition: "transform 0.3s ease", transform: open ? "rotate(180deg)" : "rotate(0deg)" }}><path d="M6 9l6 6 6-6"/></svg>
        </button>
        <div style={{ flex: 1, height: 1, background: "rgba(100,200,180,0.12)" }} />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 20, maxHeight: open ? 2000 : 0, overflow: "hidden", transition: "max-height 0.5s cubic-bezier(0.4,0,0.2,1)" }}>
        {MORE_PROJECTS.map((p, i) => (
          <div key={i} style={{ background: "#0d1318", border: "1px solid rgba(100,200,180,0.12)", borderRadius: 6, padding: "28px 32px", transition: "border-color 0.2s", position: "relative", overflow: "hidden" }} onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(77,232,194,0.28)"; e.currentTarget.querySelector(".mp-accent-bar").style.opacity = 1; }} onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(100,200,180,0.12)"; e.currentTarget.querySelector(".mp-accent-bar").style.opacity = 0; }}>
            ...
            {p.tags?.length > 0 && <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{p.tags.map(t => <span key={t} style={tagStyle(false)}>{t}</span>)}</div>}
          </div>
        ))}
      </div>
    </div>
...
export default function Portfolio() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      for (let i = sections.length - 1; i >= 0; i--) { const el = document.getElementById(sections[i]); if (el && window.scrollY >= el.offsetTop - 120) { setActiveSection(sections[i]); break; } }
    };
  ...
  return (
    <div style={S.page}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 200, background: "rgba(8,12,16,0.88)", backdropFilter: "blur(16px)", borderBottom: "1px solid rgba(100,200,180,0.1)" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto", padding: "0 32px", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div className="nav-links" style={{ display: "flex", gap: 28, alignItems: "center" }}>
            {navLinks.map(id => (
              <a key={id} href={`#${id}`} style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: activeSection === id ? "#4de8c2" : "#6b8f85", textDecoration: "none", transition: "color 0.2s" }}>{id}</a>
            ))}
            <Link to="/blog" style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: "#6b8f85", textDecoration: "none", transition: "color 0.2s", border: "1px solid rgba(100,200,180,0.2)", padding: "4px 12px", borderRadius: 4 }}
              onMouseEnter={e => { e.currentTarget.style.color = "#4de8c2"; e.currentTarget.style.borderColor = "rgba(77,232,194,0.4)"; }}
              onMouseLeave={e => { e.currentTarget.style.color = "#6b8f85"; e.currentTarget.style.borderColor = "rgba(100,200,180,0.2)"; }}>
              BLOG
            </Link>
          </div>
        </div>
      ...
      <section id="hero" style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", paddingTop: 56 }}>
        ...
        <div style={{ ...S.container, textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 15, letterSpacing: "0.18em", textTransform: "uppercase", color: "#4de8c2", marginBottom: 24, animation: "fadeUp 0.6s ease 0.2s both" }}>Deep Learning × Knowledge Graphs</div>
        ...
        <div style={S.container}>
          ...
          <div className="reveal edu-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {RESUME.education.map(ed => (
              <div key={ed.school} style={{ background: "#0d1318", border: "1px solid rgba(100,200,180,0.12)", borderRadius: 6, padding: 28, transition: "border-color 0.2s" }} onMouseEnter={e => e.currentTarget.style.borderColor = "rgba(77,232,194,0.3)"} onMouseLeave={e => e.currentTarget.style.borderColor = "rgba(100,200,180,0.12)"}>
                ...
                <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
                  <div><span style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 22, color: "#fff" }}>{ed.gpa}</span><span style={{ fontFamily: "'DM Mono', monospace", fontSize: 12, color: "#6b8f85" }}> / 10</span></div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            {RESUME.experience.map((exp, i) => (
              <div key={i} className="reveal exp-grid" style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: "0 40px", position: "relative", paddingBottom: i < RESUME.experience.length - 1 ? 48 : 0 }}>
                {i < RESUME.experience.length - 1 && <div className="exp-connector" style={{ position: "absolute", left: "calc(200px + 60px)", top: 8, bottom: 0, width: 1, background: "linear-gradient(to bottom, #4de8c2, rgba(100,200,180,0.1))" }} />}
                ...
                <div className="exp-body" style={{ paddingLeft: 48, position: "relative" }}>
                  ...
                  <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 6 }}>
                    {exp.bullets.map((b, j) => (<li key={j} style={{ fontSize: 14, color: "#6b8f85", paddingLeft: 16, position: "relative", lineHeight: 1.6 }}><span style={{ position: "absolute", left: 0, color: "#4de8c2", fontSize: 12 }}>→</span>{b}</li>))}
                  </ul>
                </div>
              </div>
            ))}
        ...
        <div style={S.container}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {RESUME.projects.map((p, i) => (
              <div key={i} className="reveal" style={{ background: "#0d1318", border: "1px solid rgba(100,200,180,0.12)", borderRadius: 6, padding: "28px 32px", transition: "border-color 0.2s", position: "relative", overflow: "hidden" }} onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(77,232,194,0.28)"; e.currentTarget.querySelector(".accent-bar").style.opacity = 1; }} onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(100,200,180,0.12)"; e.currentTarget.querySelector(".accent-bar").style.opacity = 0; }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{p.tags.map(t => <span key={t} style={S.tag(false)}>{t}</span>)}</div>
              </div>
            ))}
          </div>
          <div className="reveal"><MoreProjects tagStyle={S.tag} /></div>
        </div>
      </section>
        ...
        <div style={S.container}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
            {RESUME.skills.map((sk, i) => (
              <div key={i} className="reveal" style={{ background: "#0d1318", border: "1px solid rgba(100,200,180,0.12)", borderRadius: 6, padding: 24, transition: "border-color 0.2s, transform 0.2s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(77,232,194,0.3)"; e.currentTarget.style.transform = "translateY(-2px)"; }} onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(100,200,180,0.12)"; e.currentTarget.style.transform = ""; }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{sk.tags.map(t => <span key={t} style={S.tag(sk.purple)}>{t}</span>)}</div>
              </div>
            ))}
          </div>
        </div>
